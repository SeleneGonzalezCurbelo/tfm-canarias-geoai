...
OUTPUT_DIR = ROOT_DIR / "data" / "processed" / "renta"
...
t_start = time.time()
stats: Dict[str, Any] = {"script": "04_descarga_renta_media_hogar"}
...
def main() -> None:
    try:
        ...
        if len(gdf) == 0:
            raise ValueError("Sin datos de renta para Canarias.")
        ...
        stats["nulls_summary"] = gdf.drop(columns="geometry", errors="ignore").isna().sum().to_dict()
        ...
        print(f"\n[ERROR] El script 04 ha fallado: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        sys.exit(1)

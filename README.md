...
* **Carga validada (§1–5):** 1581×36, 0 nulos, CUSEC 10 dígitos sin duplicados. Cobertura: Tenerife 670 (42.4%), Gran Canaria 642 (40.6%), Lanzarote 87, La Palma 83, Fuerteventura 67, La Gomera 23, El Hierro 9.
...
* **K-Means exploratorio (§17b–17c, §20–24):** codo K3–K4; Sil/DB K=2 0.2372/1.5407 · K=3 0.2057/1.6075 · **K=4 0.1816/1.7019** · K=5 0.1599/1.8691. **Decisión K=4 por criterio territorial** (K=2/3 colapsan periferias; K=5 deja <10 secciones en Hierro/Gomera; OCDE 2008/Eurostat 2018: núcleo/periurbano/rural-envejecido/periferia-aislada). Perfiles §21 + radar §23 = hipótesis PG1/PG2 para el benchmark. AE/PCA sobre latente = plantillas (viven en nb 02).
...
* **Pesos espaciales island-aware (§2):** k-NN k=5 con distancia infinita inter-islas (8 componentes conectados) + verificación; base de todo Moran/LISA (contigüidad estándar no válida en archipiélago).
* **Autoencoder (§3–3.1, modelo central):** encoder 18→hidden→latente + decoder simétrico, LayerNorm (no BatchNorm: full-batch n=1581), ReLU parametrizable, MSE. **Random Search** 15 combos (`latent_dim [2,3,5]`, `hidden [8,16,32]`, `activation [relu,tanh,leaky_relu]`, `lr [1e-2,1e-3,5e-4]`, 250 epochs) con **criterio Moran's I medio** (no MSE; mejor lat3/hid32/relu/lr1e-3, Moran-val 0.8240, full 0.6154 vs PCA 0.5939, p-pareado 0.295 NS), reentreno final 500 epochs + curva pérdida.
...
* **XGBoost (§6–6.3):** RFE (DecisionTree, conserva 8/16 vars) para probar si accesibilidad/demografía predicen renta sin pobreza; **Spatial Island K-Fold** (6 folds ≥15 secciones, Hierro 9 excluida / Palma 83 incluida; Roberts et al. 2017; R² macro −0.12 / pond 0.12 vs aleatorio 0.37) + contraste split aleatorio (cuantifica inflación por autocorrelación) + importancia variables (`dist_cs_log` top). GPU `hist/cuda` si disponible.
* **Validación comparativa (§7):** Moran global medio por dimensión AE vs PCA + test permutación pareado (p=0.295 NS = equivalencia); **LISA** (High-High/Low-Low/atípicos, p<0.05) sobre dim-0 AE/PCA → `lisa_cuadrante_ae/pca` (AE 308HH/281LL/81 atípicos; PCA 436HH/303LL/13 atípicos); Silhouette/DB secundarios (AE K4 0.5795/0.55 vs PCA K2 0.35/1.08); **bootstrap** 30×80% igual N (PCA 0.98 vs AE 0.49); **ablación** por dimensión temática (salud única que baja Moran; demografía +0.17 = ruido vs suavidad); sesgo-varianza §10.1; Random Forest valida predictibilidad clusters (§7.5, AE 0.90); Fuzzy C-Means + `zona_transicion` (max_memb<0.7, 371 secciones 23.5%) para bordes difusos (§7.6).
* **Mapas (§8):** Folium archipiélago + **Lanzarote** (zoom 11, filtro exacto `isla==`, 87 secciones) por estrategia, mapa LISA, mapa renta continua, paneles estáticos `panel_comparativo_ae_pca.png` / `panel_comparativo_lanzarote.png` + radares por clúster (1 var por dimensión + perfiles).
